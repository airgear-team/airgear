...
public class UserControllerTest {
    ...
    public static void initTest() {
        ...
        userTest.setPhone("+380977777777");
        userTest.setEmail("userTest@mail.one");

        adminTest = new User();
        adminTest.setUsername("adminTest");
        adminTest.setPassword("2222");
        adminTest.setPhone("+380977777776");
        adminTest.setEmail("adminTest@mail.one");
        ...
        moderatorTest.setPhone("+380977777775");
        moderatorTest.setEmail("moderatorTest@mail.one");
    ...
    private void userAuthenticate(User user, HttpHeaders currentHeaders, Boolean isRegister){
        LoginUserDto loginUserDto = new LoginUserDto(user.getUsername(), user.getPassword());
        HttpEntity<?> entity = new HttpEntity<>(loginUserDto, currentHeaders);
        ...
        if (response.getStatusCode().equals(HttpStatus.UNAUTHORIZED)||response.getStatusCode().equals(HttpStatus.INTERNAL_SERVER_ERROR)) {
            if (isRegister) {
                HttpEntity<?> entity1 = new HttpEntity<>(userMapper.toDto(user), currentHeaders);
               String user1 = template.postForObject("http://localhost:" + port + "/auth/register", entity1, String.class);
    ...
    public void createGoodsByUser(){
        ...
        GoodsDto goodsTest1 = template.postForObject("http://localhost:" + port + "/goods", entity, GoodsDto.class);
        goodsTest = goodsMapper.toModel(goodsTest1);
